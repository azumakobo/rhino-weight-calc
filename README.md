...
## 0.7. v5 の変更点（CSV 出力 / CSV export）

選択モードの計算結果を **CSV** に保存できます（Excel / Google Sheets / 見積書・制作メモへの転用用）。計算結果の表示後に保存可否と保存先を尋ねます。

- **列**: `object_id, object_name, layer, material, density, volume, weight_kg, price_per_kg, estimated_cost`（`volume` は m³）。
- **保存先**: ダイアログで選択。キャンセル／ダイアログ不可時は **デスクトップ**（無ければホーム）へフォールバック。ファイル名は `rhino_weight_YYYYMMDD_HHMMSS.csv`。
- **文字コード**: UTF-8（BOM 付き）。Excel で日本語が文字化けしません。
- CSV 保存に失敗しても、**重量計算結果の表示は止まりません**。
- 出力例:
  ```csv
  ...
  ```
  （`price_per_kg` 未設定の素材は `price_per_kg` / `estimated_cost` が空欄になります。）

---

## 0.8. v6 の変更点（レイヤー別材料設定 / Layer materials）

大きなモデルで毎回オブジェクトごとに素材を選ばなくて済むよう、**Rhino のレイヤー名ごとに素材を割り当て**られます。割り当てはローカル設定に保存され、次回も再利用されます。

- レイヤーに素材が設定済みなら**それを優先**します（プロンプトなし）。
- 未設定のレイヤーは、**前回素材を既定**にしたプロンプトで 1 回だけ選択し、その選択を当該レイヤーに保存します（同じレイヤーの他オブジェクトでは再利用）。
- 設定ファイル例:
  ```json
  {
    "last_material": "Steel",
  ...
  ```
- 合計はレイヤー別に異なる素材を考慮して**行ごとに集計**します（使用素材一覧も表示）。
- リセット: `~/.rhino_weight_calc_settings.json` を削除すると全レイヤー割り当ても初期化されます。

---

## 0.9. v7 の変更点（Rhino UserText に材料情報を保存 / UserText metadata）

材料情報をローカル設定だけでなく、**Rhino ファイル内のオブジェクト自身**に保存できます。UserText（オブジェクトに紐づくキー・バリュー属性）を使うため、ファイルを共有・再オープンしても各オブジェクトの材料が保持されます。

- 書き込むキー: `rwc_material` / `rwc_density` / `rwc_price_per_kg`（`rwc_` プレフィックスのみ。**他の UserText には一切触れません**）。
- **素材の優先順位**:
  1. オブジェクトの UserText（`rwc_material`）
  2. レイヤー材料設定（v6）
  3. 前回素材（v2）
  4. ユーザーに選択させる
- 既知素材（`MATERIALS`）を `rwc_material` に持つ場合、`rwc_density` / `rwc_price_per_kg` 未指定なら素材表の値で補完します。独自素材名でも `rwc_density` があれば計算できます。
- 計算後、**確認ダイアログで Yes のときのみ**選択オブジェクトへ書き込みます（既存の rwc_ 以外属性は破壊しません）。
- リセット / 削除:
  - 補助関数 `clear_rwc_usertext(object_id)` は対象オブジェクトの `rwc_` キーのみ削除します（他は残す）。
  - Rhino 標準では、オブジェクト選択 → `SetUserText`（プロパティ）から個別削除も可能です。
